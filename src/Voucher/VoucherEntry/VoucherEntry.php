...
//        die();
    if (isset($_POST['ID1_description']) && !empty($_POST['ID1_description'])){
        $sl =0;
        $numberOfEntry = (count($data)-9)/6;
//        echo $numberOfEntry;
//        die();
        for ($i=0;$i<$numberOfEntry;$i++){
        $sl++;
        if(isset($data) && !empty($data)){
            $query="INSERT INTO `tbl_voucher` (`id`, `company_id`,`voucher_no`,`date`,`expense_type`,`employee_id`,`employee_name`,`employee_designation`,`project_id`,`description`,`amount`,`remarks`,`is_approved`,`is_expensed`,`entry_date`,`entry_by`) VALUES ('', '".$data['companyId']."','".$data['voucherNo']."','".$data['date']."','".$data['expenseType']."','".$data['employeeId']."','".$data['employeeName']."','".$data['designation']."','".$data['projectId']."','".$data["ID".$sl."_description"]."','". $data["ID".$sl."_amount"]."','". $data["ID".$sl."_remark"]."','0','0','". date('Y-m-d H:i:s')."','". $data["entryBy"]."')";
//            echo $query;
//            die();
...
//    die();
        for ($i=0;$i<$numberOfEntry;$i++){
            $sl++;
            if(isset($data) && !empty($data)){
                $query="INSERT INTO `tbl_voucher` (`id`, `company_id`,`voucher_no`,`date`,`expense_type`,`employee_id`,`employee_name`,`employee_designation`,`project_id`,`from_place`,`to_place`,`vehicle`,`amount`,`remarks`,`is_approved`,`is_expensed`,`entry_date`,`entry_by`) VALUES ('', '".$data['companyId']."','".$data['voucherNo']."','".$data['date']."','".$data['expenseType']."','".$data['employeeId']."','".$data['employeeName']."','".$data['designation']."','".$data['projectId']."','". $data["ID".$sl."_from"]."','". $data["ID".$sl."_to"]."','". $data["ID".$sl."_vehicle"]."','". $data["ID".$sl."_amount"]."','". $data["ID".$sl."_remark"]."','0','0','". date('Y-m-d H:i:s')."','". $data["entryBy"]."')";
//            echo $query;
//            die();
                if (mysql_query($query)) {
                    $_SESSION['successMessage'] = "Voucher Added Successfully";
...
//            die();
    ...
    public function pendingVouchers(){
        $mydata=array();
        $query="SELECT * FROM `tbl_voucher` WHERE `tbl_voucher`.`company_id`='".$this->companyId."' AND `tbl_voucher`.`is_approved`='0' AND deleted_at IS NULL GROUP BY voucher_no ORDER BY id DESC";
//        echo $query;
//        die();
        $result=  mysql_query($query);
        while ($row=  mysql_fetch_assoc($result)){
...
//        header('location:index.php');
    ...
    public function lastEntry(){
        $query="SELECT * FROM `tbl_voucher` WHERE `tbl_voucher`.`company_id` = '".$this->companyId."' ORDER BY id DESC LIMIT 1";
//        echo $query;
//        die();
        $result=  mysql_query($query);
